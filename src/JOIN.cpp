...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/31 22:31:32 by moamzil           #+#    #+#             */
/*   Updated: 2025/01/31 22:31:34 by moamzil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void CommandHandler::execute_JOIN(Client &client, Channel &Mainchannel)
{
    ...
    if (channel == nullptr)
    {
        ...
        if (!channel->getPassword().empty() && command_args.size() != 3)
        ...
        else if (channel->isUserInChannel(&client))
        {
            sendMessageToClient(client, "You are already in the channel: " + channelName + "\n");
            command_args.clear();
            return;
        }
        if (channel->getInviteOnly() && !client.checkInvite(channel->getName()))
        {
            sendMessageToClient(client, "You Must have an Invitation to Join : " + channelName + "\n");
            command_args.clear();
            return;
        }
